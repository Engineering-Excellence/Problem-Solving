...
int n;
array<int, 100'001> idx;
array<int, 100'001> inorder;
array<int, 100'001> postorder;
...
void input() {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> inorder[i];
        idx[inorder[i]] = i; // inorder 요소들의 idx 정보 저장
    }
    for (int i = 1; i <= n; i++)
        cin >> postorder[i];
}
...
void getPreOrder(int inStart, int inEnd, int postStart, int postEnd) {
    ...
    // idx 배열을 통해 inorder에서의 root index를 쉽게 구할 수 있다.
